...
{
    public partial class ExpectedInterviews : Form
    {
        public ExpectedInterviews()
        {
            InitializeComponent();
            refresh();
        }
            ...
            switch (Properties.Settings.Default.l)
            {
                case 21:
                    ...
                    SqlDataAdapter dataAdapter18 = new SqlDataAdapter
                        ...
                        "AND Собеседования.Код_Сотрудника = Сотрудники.Код_Сотрудника AND Собеседования.Код_Статуса = Статус_Собеседования.Код_Статуса AND Статус_Собеседования.Код_Статуса LIKE 3", connection18);
            ...
            if (e.RowIndex != -1)
                try
                {
                    ...
                    switch (Properties.Settings.Default.l)
                    {
                        case 21:
                            {
                                PassingTheInterview acceptAnInterview = new PassingTheInterview(id);
                                acceptAnInterview.ShowDialog();
                            }
                            break;
                    }
