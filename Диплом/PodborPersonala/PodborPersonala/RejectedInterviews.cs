...
{
    public partial class RejectedInterviews : Form
    {
        public RejectedInterviews()
        {
            InitializeComponent();
            refresh();
        }
            ...
            switch (Properties.Settings.Default.l)
            {
                case 22:
                    ...
                    SqlDataAdapter dataAdapter18 = new SqlDataAdapter
                        ...
                        "AND Собеседования.Код_Сотрудника = Сотрудники.Код_Сотрудника AND Собеседования.Код_Статуса = Статус_Собеседования.Код_Статуса AND Статус_Собеседования.Код_Статуса LIKE 2", connection18);
            ...
            if (e.RowIndex != -1)
                try
                {
                    ...
                    switch (Properties.Settings.Default.l)
                    {
                        case 22:
                            {
                                DeleteInterviews deleteInterviews = new DeleteInterviews(id);
                                deleteInterviews.ShowDialog();
                            }
                            break;
                    }
